...
class ProgressTests(unittest.TestCase):
    ...
    def test_work_item_ids_and_dependencies(self):
        items=json.loads((ROOT/'data/work_items.json').read_text())['items'];ids={i['id'] for i in items}
        self.assertEqual(len(ids),len(items))
        for item in items:
            self.assertTrue(set(item['depends_on'])<=ids)
            self.assertNotIn(item['id'],item['depends_on'])
            self.assertTrue(item['acceptance']);self.assertIsNone(item['github_issue_number'])
